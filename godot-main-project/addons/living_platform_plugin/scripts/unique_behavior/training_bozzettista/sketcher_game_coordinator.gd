...
@export var stargates_to_hide_at_start: Array[LivingObject] = []
@export var stargates_to_show_at_end: Array[LivingObject] = []
...
func _hide_stargates(stargates: Array[LivingObject]) -> void:
	for stargate in stargates:
		if stargate != null and is_instance_valid(stargate):
			stargate.hide()


func _show_stargates(stargates: Array[LivingObject]) -> void:
	for stargate in stargates:
		if stargate != null and is_instance_valid(stargate):
			stargate.show()
