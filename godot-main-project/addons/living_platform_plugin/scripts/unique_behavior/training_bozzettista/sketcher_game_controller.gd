...
@export var source_slideshow_element: LivingObject
...
const _SLIDESHOW_PANEL_SCRIPT := preload(
	"res://addons/living_platform_plugin/scripts/unique_behavior/training_bozzettista/sketcher_game_slideshow_panel.gd"
)
...
func _find_slideshow_element_in_area(area: LivingArea) -> LivingObject:
	for child in area.get_children():
		if child is SketcherGameController:
			continue
		if not child is LivingObject:
			continue
		for grandchild in child.get_children():
			if grandchild is LivingSlideShow:
				return child as LivingObject
	return null
