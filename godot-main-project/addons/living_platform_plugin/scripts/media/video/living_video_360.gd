...
@export var sphere_radius: float = 500.0:
	set(value):
		sphere_radius = max(value, 0.1)
		if is_inside_tree() and _mesh_instance != null:
...
func apply_settings(settings: Dictionary) -> void:
	if settings.has("sphere_radius"):
		sphere_radius = float(settings["sphere_radius"])


func _apply_sphere_radius() -> void:
	if _mesh_instance == null or not (_mesh_instance.mesh is SphereMesh):
		return
