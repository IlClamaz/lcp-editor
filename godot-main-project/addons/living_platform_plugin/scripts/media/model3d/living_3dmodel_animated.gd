...
func apply_settings(settings: Dictionary) -> void:
	if settings.has("move_speed"):
		move_speed = float(settings["move_speed"])
		current_speed = move_speed
	if settings.has("random_poses_playing"):
		random_poses_playing = bool(settings["random_poses_playing"])
	if settings.has("moving"):
		moving = bool(settings["moving"])
	if settings.has("random_spawn"):
		random_spawn = bool(settings["random_spawn"])
	if settings.has("extra_pose_chain_chance"):
		extra_pose_chain_chance = float(settings["extra_pose_chain_chance"])
	if settings.has("extra_pose_chain_min"):
		extra_pose_chain_min = int(settings["extra_pose_chain_min"])
	if settings.has("extra_pose_chain_max"):
		extra_pose_chain_max = int(settings["extra_pose_chain_max"])
