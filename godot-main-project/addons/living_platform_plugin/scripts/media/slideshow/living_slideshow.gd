...
func apply_settings(settings: Dictionary) -> void:
	if settings.has("loop_slides"):
		loop_slides = bool(settings["loop_slides"])
	if settings.has("slide_transition_enabled"):
		slide_transition_enabled = bool(settings["slide_transition_enabled"])
	if settings.has("slide_transition_duration"):
		slide_transition_duration = float(settings["slide_transition_duration"])
	if settings.has("slide_transition_fade_min_alpha"):
		slide_transition_fade_min_alpha = float(settings["slide_transition_fade_min_alpha"])
	if settings.has("auto_hide_source_elements"):
