...
## Applies current session state to scene nodes (stargates, highlight lights).
func sync_scene_presentation_from_session() -> void:
	...
	_sync_stargates_from_session(env)
	_sync_lights_from_session(env)


func _sync_stargates_from_session(env: LivingEnvironment) -> void:
	var registry := LivingSessionManager.get_registry()
	for node in env.find_children("*", "LivingStargate", true, false):
		if not node is LivingStargate:
			continue
		var stargate := node as LivingStargate
		var item_id: int = stargate.get_parent().item_id
		if item_id <= 0:
			continue
		...
		if activation == "" and use_state == "":
			continue
		stargate.apply_presentation_state(activation, use_state)
		if activation == "ACTIVE" or use_state == "USED": # To keep training stargates open after training completion.
			stargate.get_parent().show()
	...
	match event.action:
		...
		LivingEvent.ActionType.ACTIVATE_TRIGGER:
			...
			for target_trigger_id in event.action_params:
				for node in env.find_children("*", "LivingStargate", true, false):
					if node is LivingStargate and node.get_parent().item_id == target_trigger_id:
						if node._stargate_state == LivingStargate.StargateState.INACTIVE: # In case some event is retriggered.
							node.activate()
							break
		...
		LivingEvent.ActionType.PLAY_VIDEO_360:
			...
			for node in env.find_children("*", "LivingVideo360", true, false): 
				# SIA QUESTO CHE LO STARGATE VERRANNO SOSTITUITI DAL LIVINGSTARGATEOBJ e LIVING360VIDEOOBJ
