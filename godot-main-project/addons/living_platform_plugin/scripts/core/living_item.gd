...
# ==============================================================================
const OMEKA_TITLE_MAX_LEN: int = LivingConstants.OMEKA_TITLE_MAX_LEN
var MEDIA_SAVE_PATH: String = "res://downloaded_living_media"
var living_video_player_scene = preload("res://addons/living_platform_plugin/scripts/media/video/living_video.tscn")
const LIVING_SLIDESHOW_SCENE_PATH := "res://addons/living_platform_plugin/scripts/media/slideshow/living_slideshow.tscn"
...
## Prefetch tree from Fase 0: item_id -> friendly meta (no Omeka field names).
## When present, Fase 1 applies metadata from here and skips per-item GET.
var _omeka_tree: Dictionary = {}
...
func _run_build_process_async() -> void:
	...
	# crea subito i loro nodi base (es. LivingObject-1739) e attaccateli sotto".
...
func _fetch_omeka_metadata_async() -> bool:
	# Prefer prefetched friendly meta from Fase 0 (no extra HTTP).
	var cached := _get_prefetched_meta_for_self()
	if not cached.is_empty():
		_apply_prefetched_meta(cached)
		return true
	...
	# Parse via prefetcher so LivingItem never knows Omeka field names.
	var meta := OmekaTreePrefetcher.new().parse_item_metadata(item_dict)
	_apply_prefetched_meta(meta)
	return true


func _get_prefetched_meta_for_self() -> Dictionary:
	if item_id <= 0 or _omeka_tree.is_empty():
		return {}
	if not _omeka_tree.has(item_id):
		return {}
	var meta = _omeka_tree[item_id]
	if typeof(meta) != TYPE_DICTIONARY or meta.is_empty():
		return {}
	return meta


# Applies friendly meta from OmekaTreePrefetcher (no Omeka JSON keys here).
func _apply_prefetched_meta(meta: Dictionary) -> void:
	title = str(meta.get(LivingConstants.OMEKA_META_TITLE, LivingConstants.OMEKA_DEFAULT_ITEM_TITLE))
	self.name = title.substr(0, OMEKA_TITLE_MAX_LEN)
	modified = str(meta.get(LivingConstants.OMEKA_META_MODIFIED, ""))
	short_description = str(meta.get(LivingConstants.OMEKA_META_SHORT_DESCRIPTION, ""))
	long_description = str(meta.get(LivingConstants.OMEKA_META_LONG_DESCRIPTION, ""))
	catalog_description = str(meta.get(LivingConstants.OMEKA_META_CATALOG_DESCRIPTION, ""))
	resource_class = int(meta.get(LivingConstants.OMEKA_META_RESOURCE_CLASS, 0))
	participatory_item_type = str(meta.get(LivingConstants.OMEKA_META_PARTICIPATORY_ITEM_TYPE, ""))
	medium_uri = str(meta.get(LivingConstants.OMEKA_META_MEDIUM_URI, ""))
	thumbnail_uri = str(meta.get(LivingConstants.OMEKA_META_THUMBNAIL_URI, ""))

	components.clear()
	for cid in meta.get(LivingConstants.OMEKA_META_COMPONENTS, []):
		components.append(int(cid))
	areas.clear()
	for aid in meta.get(LivingConstants.OMEKA_META_AREAS, []):
		areas.append(int(aid))

	notify_property_list_changed()


func _participatory_type_from_tree(component_id: int) -> String:
	if not _omeka_tree.has(component_id):
		return ""
	var meta = _omeka_tree[component_id]
	if typeof(meta) != TYPE_DICTIONARY:
		return ""
	return str(meta.get(LivingConstants.OMEKA_META_PARTICIPATORY_ITEM_TYPE, ""))


# Uses _omeka_tree for typed spawn/replace when available.
# If tree is empty (manual Instantiate Children), creates generic LivingObject and does not replace.
# If type is known and an existing child has the wrong class, replace it keeping transform + LivingObject props.
func instantiate_children() -> void:
	...
	for c in components:
		...
		if existing_children.has(c):
			var old_child: LivingItem = existing_children[c]
			# Senza tipo noto non sostituiamo (evita replace aggressivi da bottone manuale).
			if item_type != "" and not _component_matches_type(old_child, item_type):
				print(
					"LivingItem: type mismatch for id %d (have %s, want '%s') — replacing node, keeping transform."
					% [c, old_child.get_class(), item_type]
				)
				_replace_living_component(old_child, c, item_type)
			existing_children.erase(c)
		else:
			var new_element := _create_living_component(c, item_type)
...
func _component_matches_type(node: LivingItem, participatory_item_type: String) -> bool:
	match participatory_item_type:
		LivingConstants.PARTICIPATORY_TYPE_SUONO:
		...
		LivingConstants.PARTICIPATORY_TYPE_CROWD:
			return node is LivingCrowdObject
		LivingConstants.PARTICIPATORY_TYPE_MODELLO_CONTENITORE:
			return node is LivingContainerModelObject
		LivingConstants.PARTICIPATORY_TYPE_STARGATE:
			return node is LivingStargateObject
		LivingConstants.PARTICIPATORY_TYPE_IMMAGINE:
			return node is LivingImageObject
		LivingConstants.PARTICIPATORY_TYPE_VIDEO:
			return node is LivingVideoObject
		LivingConstants.PARTICIPATORY_TYPE_SLIDESHOW:
			return node is LivingSlideShowObject
		LivingConstants.PARTICIPATORY_TYPE_VIDEO360:
			return node is LivingVideo360Object
		_:
			# Typed subclasses also `is LivingObject` — exclude them from the generic bucket.
			return node is LivingObject and not _is_typed_living_object(node)


func _is_typed_living_object(node: LivingItem) -> bool:
	return (
		...
		or node is LivingStargateObject
		or node is LivingFlatMediaObject
		or node is LivingVideo360Object
	)


# Creates the correct LivingItem subclass for a component id.
# Extend this match as new typed parents are introduced (Video, Image, …).
func _create_living_component(component_id: int, participatory_item_type: String) -> LivingItem:
	var node: LivingItem
	match participatory_item_type:
		LivingConstants.PARTICIPATORY_TYPE_SUONO:
			node = LivingAudioObject.new()
			node.name = "LivingAudioObject-" + str(component_id)
		LivingConstants.PARTICIPATORY_TYPE_OGGETTO:
		...
		LivingConstants.PARTICIPATORY_TYPE_CROWD:
			node = LivingCrowdObject.new()
			node.name = "LivingCrowdObject-" + str(component_id)
		LivingConstants.PARTICIPATORY_TYPE_MODELLO_CONTENITORE:
			node = LivingContainerModelObject.new()
			node.name = "LivingContainerModelObject-" + str(component_id)
		LivingConstants.PARTICIPATORY_TYPE_STARGATE:
			node = LivingStargateObject.new()
			node.name = "LivingStargateObject-" + str(component_id)
		LivingConstants.PARTICIPATORY_TYPE_IMMAGINE:
			node = LivingImageObject.new()
			node.name = "LivingImageObject-" + str(component_id)
		LivingConstants.PARTICIPATORY_TYPE_VIDEO:
			node = LivingVideoObject.new()
			node.name = "LivingVideoObject-" + str(component_id)
		LivingConstants.PARTICIPATORY_TYPE_SLIDESHOW:
			node = LivingSlideShowObject.new()
			node.name = "LivingSlideShowObject-" + str(component_id)
		LivingConstants.PARTICIPATORY_TYPE_VIDEO360:
			node = LivingVideo360Object.new()
			node.name = "LivingVideo360Object-" + str(component_id)
		_:
			node = LivingObject.new()
			node.name = "LivingObject-" + str(component_id)
	node.item_id = component_id
	if participatory_item_type != "":
		node.participatory_item_type = participatory_item_type
	return node


func _replace_living_component(old_node: LivingItem, component_id: int, participatory_item_type: String) -> void:
	var snap := _snapshot_living_component(old_node)
	var child_index := old_node.get_index()
	var previous_owner := old_node.owner

	old_node.owner = null
	remove_child(old_node)
	old_node.queue_free()

	var fresh := _create_living_component(component_id, participatory_item_type)
	_restore_living_component(fresh, snap)
	add_child(fresh)
	move_child(fresh, child_index)
	if Engine.is_editor_hint():
		fresh.owner = previous_owner if previous_owner != null else get_tree().edited_scene_root


func _snapshot_living_component(node: LivingItem) -> Dictionary:
	var snap := {
	...
	if node is LivingFlatMediaObject:
		...
		snap["diagonal"] = flat.diagonal
	if node is Living3DModelObject:
		snap["face_visible"] = (node as Living3DModelObject).face_visible
	if node is Living3DModelAnimatedObject:
		var anim := node as Living3DModelAnimatedObject
		snap["face_visible"] = anim.face_visible
		snap["move_speed"] = anim.move_speed
		snap["random_poses_playing"] = anim.random_poses_playing
		snap["moving"] = anim.moving
		snap["random_spawn"] = anim.random_spawn
		snap["extra_pose_chain_chance"] = anim.extra_pose_chain_chance
		snap["extra_pose_chain_min"] = anim.extra_pose_chain_min
		snap["extra_pose_chain_max"] = anim.extra_pose_chain_max
	if node is LivingCrowdObject:
		snap["density"] = (node as LivingCrowdObject).density
	for meta_key in [LivingConstants.META_IS_BORN, LivingConstants.META_EDIT_GROUP, LivingConstants.META_EDIT_LOCK]:
		if node.has_meta(meta_key):
			snap["meta_%s" % meta_key] = node.get_meta(meta_key)
	return snap


func _restore_living_component(node: LivingItem, snap: Dictionary) -> void:
	...
	if node is LivingFlatMediaObject:
		...
		if snap.has("curvature"):
			flat.curvature = snap["curvature"]
		if snap.has("diagonal"):
			flat.diagonal = snap["diagonal"]
	if node is Living3DModelObject and snap.has("face_visible"):
		(node as Living3DModelObject).face_visible = snap["face_visible"]
	if node is Living3DModelAnimatedObject:
		var anim := node as Living3DModelAnimatedObject
		if snap.has("face_visible"):
			anim.face_visible = snap["face_visible"]
		if snap.has("move_speed"):
			anim.move_speed = snap["move_speed"]
		if snap.has("random_poses_playing"):
			anim.random_poses_playing = snap["random_poses_playing"]
		if snap.has("moving"):
			anim.moving = snap["moving"]
		if snap.has("random_spawn"):
			anim.random_spawn = snap["random_spawn"]
		if snap.has("extra_pose_chain_chance"):
			anim.extra_pose_chain_chance = snap["extra_pose_chain_chance"]
		if snap.has("extra_pose_chain_min"):
			anim.extra_pose_chain_min = snap["extra_pose_chain_min"]
		if snap.has("extra_pose_chain_max"):
			anim.extra_pose_chain_max = snap["extra_pose_chain_max"]
	if node is LivingCrowdObject and snap.has("density"):
		(node as LivingCrowdObject).density = snap["density"]
	for meta_key in [LivingConstants.META_IS_BORN, LivingConstants.META_EDIT_GROUP, LivingConstants.META_EDIT_LOCK]:
		var snap_key := "meta_%s" % meta_key
		if snap.has(snap_key):
			node.set_meta(meta_key, snap[snap_key])
...
func _participatory_type_is_structural(item_type: String) -> bool:
	return item_type in [
		LivingConstants.PARTICIPATORY_TYPE_AMBIENTE,
		LivingConstants.PARTICIPATORY_TYPE_AREA,
	]
...
func instantiate_medium() -> void:
	...
	# se è di tipo Stargate, non guardiamo il media_path ma cerchiamo direttamente se ha già un figlio stargate, 
	...
	if participatory_item_type == LivingConstants.PARTICIPATORY_TYPE_STARGATE:
	...
	match participatory_item_type:
		...
		LivingConstants.PARTICIPATORY_TYPE_MODELLO_CONTENITORE:
			...
			self.set_meta(LivingConstants.META_EDIT_LOCK, true)
			new_child.set_meta(LivingConstants.META_EDIT_LOCK, true)
			new_child.extraction_dir = extract_dir
			new_child.entry_scene_path = extract_dir.path_join(LivingConstants.LIVING_ENVIRONMENT_TEMPLATE_FILENAME)
		LivingConstants.PARTICIPATORY_TYPE_SLIDESHOW:
		...
		LivingConstants.PARTICIPATORY_TYPE_STARGATE:
			new_child = LivingStargate.new()
			new_child.name = "LivingStargate-" + str(item_id)
		LivingConstants.PARTICIPATORY_TYPE_SUONO:
