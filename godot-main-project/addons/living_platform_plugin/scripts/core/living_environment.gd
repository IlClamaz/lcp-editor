...
func rebuild_environment():
	...
	# =======================================================
	# FASE 0: Prefetch albero Omeka (tipi noti prima dello spawn)
	# =======================================================
	print("LivingEnvironment: Avvio FASE 0 (Prefetch albero Omeka)...")
	var prefetch_ok := await _prefetch_omeka_tree()
	if not prefetch_ok:
		_rebuild_in_progress = false
		if build_finished.is_connected(_on_rebuild_guard_finished):
			build_finished.disconnect(_on_rebuild_guard_finished)
		rebuild_completed.emit(false)
		return
...
# Fase 0: download full Omeka tree into _omeka_tree (friendly meta only).
func _prefetch_omeka_tree() -> bool:
	if item_id <= 0:
		push_error("LivingEnvironment: prefetch failed — invalid item_id.")
		return false

	var base_url := str(OMEKA_BASE_URL).strip_edges().trim_suffix("/")
	if base_url == "":
		push_error("LivingEnvironment: prefetch failed — OMEKA_BASE_URL is empty.")
		return false

	var prefetcher := OmekaTreePrefetcher.new()
	prefetcher.progress.connect(func(fetched: int, queued: int):
		import_progress.emit("Prefetch Omeka: %d done, %d queued" % [fetched, queued])
	)

	var result := await prefetcher.prefetch_tree(self, base_url, item_id)
	if not result.get("ok", false):
		push_error("LivingEnvironment: prefetch failed — %s" % str(result.get("error", "unknown")))
		_omeka_tree.clear()
		return false

	_omeka_tree = result.get("tree", {})
	print("LivingEnvironment: Prefetch OK — %d items." % _omeka_tree.size())
	return true
